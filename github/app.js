...
function verificarToken(req, res, next) {
  const token = req.cookies.token; // Obtén el token de las cookies (asegúrate de tener el middleware cookieParser configurado)
  
  if (token) {
    jwt.verify(token, 'secreto', (error, decoded) => {
      if (error) {
        res.redirect('/Index.html'); // Redirecciona a Index.html si el token no es válido
      } else {
        // El token es válido, continúa con la siguiente función de middleware
        next();
      }
    });
  } else {
    res.redirect('/Index.html'); // Redirecciona a Index.html si no se proporciona un token
  }
}
...
app.get('/logout', (req, res) => {
  ...
  res.redirect('/Index.html'); // Redireccionar a Index.html
});
// Iniciar servidor
app.listen(PORT, () => {
